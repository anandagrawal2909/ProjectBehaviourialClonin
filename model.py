            ...
            for i in range(3):
                for rows in range(start, start + batch_size):
                    if rows < DATA_SIZE:
                        ...
                        #if measurement != 0:
                        image = imread(data[POSITION[i]][row].strip()) # Reading images and remove whitespace in image path
...
history_object = model.fit_generator(train_generator, steps_per_epoch= len(TRAIN_DATA), validation_data=validation_generator, validation_steps=len(VALIDATION_DATA), epochs=5, verbose = 1)
